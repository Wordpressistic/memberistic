...
`Wordpressistic/memberistic` is the official product repository and the public
home of Memberistic. Releases are cut here, and this is the tree that becomes
the distributable plugin. `Shubochandrosarker/memberistic` is a personal
full-history working copy used for planning and drafting; work that lands in
the product is ported here deliberately, never merged wholesale.
