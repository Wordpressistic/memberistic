...
## This repository's relationship to the personal working copy

This repository — `Wordpressistic/memberistic` — is the official product
repository: the public home of Memberistic, the tree releases are cut from, and
the tree that becomes the distributable plugin.

`Shubochandrosarker/memberistic` is a personal full-history working copy used
for planning and drafting. It shares the 2.0.0 baseline and the
`public_release_v_2.0.0` commit.

Decide per change where it originates and port deliberately — do not let the
two diverge silently, and never merge the working copy's tree wholesale.
Governance, strategy and release-engineering material ports freely; runtime
plugin code is evaluated change by change, on its own evidence, with its own
tests and its own PR.

```bash
git remote add personal https://github.com/Shubochandrosarker/memberistic
git fetch personal
git log --oneline main..personal/main   # what the working copy has that we do not
git log --oneline personal/main..main   # what we have that it does not
```
